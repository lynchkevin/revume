...
function (Teams,Users,$q,$ionicPopover,$rootScope) {
    ...
    $.refreshTeams = function($scope){
        ...
        if(_id != undefined){
            $.getAll({id:_id}).then(function(teams){
                $scope.teams = teams;
                defer.resolve();
            }); 
        };
    ...
    $.editTeam = function($scope,idx){
        ...
        team.members.forEach(function(member){
            ...
            $scope.addTeam.memberString+=member.firstName+','+member.lastName+','+member.email+';';
        });
        $scope.addTeam.nameString ='';
        $scope.addTeam.emailString='';
        $scope.addTeam._id = undefined;
    ...
    $.addMember = function($scope){
        ...
        $scope.addTeam.memberString+= m.firstName+','+m.lastName+','+m.email+';';
        ...
        team.members.forEach(function(member){
            ...
            if(member._id == undefined){
                ...
                usr.$save().then(function(usr){
                    dbTeam.members.push(usr._id);
                    d.resolve();
                });
            }
            else{ //if not a new user then just push the _id
                dbTeam.members.push(member._id);
            }
        })
